...
inline std::string read_widechar(const std::uintptr_t address, const std::size_t size) {
	...
	memory::copy_memory(address, (uintptr_t)buffer.get(), size);
...
namespace mono {
	inline mono_root_domain_t* get_root_domain() {
		return reinterpret_cast<mono_root_domain_t*>(memory::read<uintptr_t>(mono_dll_address /* module handle of mono-2.0-bdwgc.dll */ + 0x499c78));
	}
